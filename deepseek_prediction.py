...
class DeepSeekPredictor:
    ...
    def prepare_data(self):
        ...
        # 创建滞后特征
        self.df['prev_interval'] = self.df['interval_days'].shift(1)
        self.df['prev_2_interval'] = self.df['interval_days'].shift(2)
        
        # 移动平均特征
        self.df['ma_3_interval'] = self.df['interval_days'].rolling(window=3).mean()
        self.df['ma_5_interval'] = self.df['interval_days'].rolling(window=5).mean()
    ...
    def arima_forecast(self):
        ...
        # 使用最佳参数训练模型
        model = ARIMA(intervals, order=best_params)
        fitted_model = model.fit()
    ...
    def create_visualizations(self):
        ...
        if not monthly_counts.empty:
            pivot_monthly = monthly_counts.pivot(index='year', columns='month', values='count').fillna(0)
            
            fig.add_trace(
                go.Heatmap(
                    z=pivot_monthly.values,
                    x=[f'{i}月' for i in pivot_monthly.columns],
